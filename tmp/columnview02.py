...
class MyColumnView(Gtk.Box):
    ...
    def __init__(self):
        ...
        # Sorters
        self.filepath_sorter = Gtk.CustomSorter.new(sort_func=self._sort_filepath_func)
        self.check_sorter = Gtk.CustomSorter.new(sort_func=self._sort_check_func)

        # Setup columnview columns
        column_label = Gtk.ColumnViewColumn.new("File path", factory_label)
        column_label.set_sorter(self.filepath_sorter)
        column_label.set_expand(True)
        column_check = Gtk.ColumnViewColumn.new("check", factory_check)
        column_check.set_sorter(self.check_sorter)
        ...
        # Setup model following instructions from example in:
        # https://docs.gtk.org/gtk4/method.ColumnView.get_sorter.html
        ...
        selection = Gtk.SingleSelection.new(self.filter_model)
    ...
    def _sort_filepath_func(self, item1, item2, data):
    ...
    def _sort_check_func(self, item1, item2, data):
