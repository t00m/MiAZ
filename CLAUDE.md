...
- **Date detection**: `util.dates_from_text` reads only dates whose field order the text settles by itself (year first, or a year-last date with a component over 12). `03_04_2024` is ambiguous, so nothing is read. `util.filename_guess_date` chains `util.dates_from_metadata` → concept hint → `UNKNOWN_DATE` (`99991231`). Metadata first is deliberate: invoice and ID numbers are digit runs that read as valid dates (`RG240719880042` yields 24 Jul 1988), while a `CreationDate` field holds a date or nothing. There is deliberately **no mtime fallback**: it made every dateless document read as imported today. `dates_from_metadata` reads PDF `/CreationDate`, PDF/image XMP `<xmp:CreateDate>`, JPEG EXIF `DateTimeOriginal` and OOXML/OpenDocument creation dates **from the raw bytes**, with no third-party library: the previous `pypdf`/`Pillow` imports were behind `except ImportError: return ''`, neither is a MiAZ dependency, so that branch was dead on every normal install. Both rename paths use it; `rename.py`'s `detect_date()` re-reads from the current concept text, wired to the "Detect date" item in the rename dialog's grouped Detect menu (see next bullet).
- **Field detection (no AI)**: `MiAZ.backend.extract` (no GTK imports) is the core, non-AI counterpart to `MiAZAIAssistant`'s LLM suggestions. `extract(path)` shells out to `pdftotext`/`pdftoppm`/`tesseract` (poppler-utils + tesseract are hard package dependencies, `miaz.spec`/`debian/control`; `missing_tools()` reports absent binaries so a dev install or the dependency-less AppImage can show an install-command dialog instead of failing silently) to get the document's text, falling back to OCR when there is no text layer; `match_vocab(text, used)` matches that text against a repository's used vocabulary (`MiAZConfig.load_used()`, key→description) for a field. `widgets/rename.py` exposes `detect_country()`, `detect_sentby()`, `detect_sentto()` (each runs one `extract()` + `match_vocab()` pass in the background via `run_in_background`) and `detect_all()` (one extraction, every field). Group, Purpose and Concept are not guessed: they are open vocabulary, where a wrong guess is harder to notice than a missing one. `services/actions.py::build_detect_menu()` builds this once as a shared `Gio.Menu` (`rename-detect-*` actions, same "build once, resolve the current rename widget dynamically" pattern as `MiAZMassRename.build_menu`) behind one `Gtk.MenuButton` ("Detect") in the rename dialog's action bar, next to Suggest/Preview. `MiAZAIAssistant`'s `miazai/extractor.py` now delegates to `MiAZ.backend.extract` for every format except `.docx`, which stays plugin-only (python-docx is not a core dependency).
